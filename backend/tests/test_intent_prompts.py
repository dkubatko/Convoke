...
def test_render_episodes_shows_confidence_on_sub_bar_slots():
    ...
    assert "time=11am" in out
    assert "time=11am (confidence" not in out
...
def test_render_episodes_all_confirmed_has_no_marker():
    ep = _episode(slots={"date": _slot("Tue", 0.9)})
    assert "(confidence" not in render_episodes([ep], 0.7, NOW, GRACE, PER_HOUR)
